...
class BookingController extends AbstractController
{
    ...
    public function createBooking(Request $request): Response
    {
        ...
        if (
            ...
            throw new HttpException(Response::HTTP_BAD_REQUEST, 'Missing required params: user_id, house_id or comment!');
        }
        ...
        if (!$user || !$house) {
            throw new HttpException(Response::HTTP_NOT_FOUND, "User $userId or house $houseId not found!");
        }

        if (!$houseRepositotry->isHouseAvailable($houseId)) {
            throw new HttpException(Response::HTTP_BAD_REQUEST, "House $houseId is not available now!");
        }
